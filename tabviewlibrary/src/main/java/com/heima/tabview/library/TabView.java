...
public class TabView extends RelativeLayout {
    ...
    private void initDefaultAttrs(Context context){
        mTextViewSelColor= Color.rgb(33,145,231);
        ...
        mTextViewSize=TabViewUtil.sp2px(context,10);
    ...
    private void initTabChildView(boolean isOutState){
        tabview.removeAllViews();
        unselectedIconList=new ArrayList<>();
        mFragments=new Fragment[mTabViewChildList.size()];

        if (isOutState) {//内存是否重启
            for(int i=0;i<mFragments.length;i++){
                mFragments[i]=mSupportFragmentManager.findFragmentByTag("" + i);
            }
        } else {
            ...
            for(int i=0;i<mFragments.length;i++){
                ...
                trx.add(R.id.tabview_fragment_container, mFragments[i],""+i);
                if (i == mTabViewDefaultPosition) {
                    trx.show(mFragments[i]).commitAllowingStateLoss();
                } else {
                    trx.hide(mFragments[i]);
                }
            }
        }
    ...
    public void setTabViewChild(List<TabViewChild> tabViewChildList, FragmentManager supportFragmentManager,boolean isOutState){
    ...
    private void showOrHide() {
        if (currentTabIndex != index) {
            ...
            if (!mFragments[index].isAdded()) {
                trx.add(R.id.tabview_fragment_container, mFragments[index],""+index);
            }
